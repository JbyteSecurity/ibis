			...
			<input type="text" class="input-block-level" id="username" name="username" placeholder="Nro Identificacion" required>
			...
			jQuery("#signin_student").submit(function(e){
					...
					$.ajax({
						...
						if(html=='true')
						{
						$.jGrowl("Welcome to Ibis Learning Management System", { header: 'Registrar Success' });
						var delay = 2000;
							setTimeout(function(){ window.location = 'dashboard_student.php'  }, delay);  
						}else if(html=='false'){
							$.jGrowl("student does not found in the database Please Sure to Check Your ID Number or Firstname, Lastname and the Section You Belong. ", { header: 'Registrar Failed' });
						}
						}
						...
						{
						$.jGrowl("student does not found in the database", { header: 'Registrar Failed' });
						}
				});
			});
			</script>
			...
			<a onclick="window.location='index.php'" id="btn_login" name="login" class="btn" type="submit"><i class="icon-signin icon-large"></i> Click para Iniciar Sesión</a>
